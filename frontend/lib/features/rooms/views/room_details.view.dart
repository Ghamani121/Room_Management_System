...
class CapacitySection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        const Icon(Icons.reduce_capacity, color: RoomDetailsView.primaryRed),
...
class EquipmentSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Icon(Icons.build, color: RoomDetailsView.primaryRed),
        const SizedBox(width: 12),
        Expanded(
          child: Text(
...
class CreatedAtSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        const Icon(Icons.calendar_today, color: RoomDetailsView.primaryRed),
        const SizedBox(width: 12),
        Text(
...
class UpdatedAtSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        const Icon(Icons.update, color: RoomDetailsView.primaryRed),
        const SizedBox(width: 12),
        Text(
          date != null ? formatter.format(date!) : "-",
          style: const TextStyle(fontSize: 16, color: Colors.black87),
...
/// Remove Room Button
class RemoveRoomButton extends StatelessWidget {
  const RemoveRoomButton({super.key});
    ...
    return Center(
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          backgroundColor: RoomDetailsView.primaryRed,
